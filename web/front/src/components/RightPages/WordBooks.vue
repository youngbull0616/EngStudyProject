<template>
  ...
  <el-card class="box-card" v-for="(item, index) in booksInfo" :key="index" style="margin-bottom: 20px;">
    <div slot="header" class="clearfix">
      ...
      <button @click="updateWordTrans">更新单词意思</button>
    </div>
...
export default {
  ...
  methods: {
    ...
    change: function (bookTitle) { // 后端更改cookie
      $.ajax({
        type: 'GET',
        url: 'http://localhost:8081/Hello/change_book/' + bookTitle,
        crossDomain: true,
        xhrFields: {
  ...
  mounted: function () {
    $.ajax({
      type: 'GET',
      url: 'http://localhost:8081/Hello/get_books/' + getCookie('username'),
      crossDomain: true,
      xhrFields: {
        withCredentials: true
      },
