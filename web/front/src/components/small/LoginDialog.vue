  ...
  methods: {
    sendEmail: function () {
      if (this.form.username === '') {
        ...
        $.ajax({
          type: 'GET',
          url: 'http://localhost:8081/Hello/resetPass/' + this.form.username,
          crossDomain: true,
          xhrFields: {
    ...
    submit_again (encrypted) { // 加密后再次把密码发给后端
      ...
      $.ajax({
        type: 'POST',
        url: 'http://localhost:8081/Hello/login_check',
        crossDomain: true,
        xhrFields: {
          withCredentials: true
        },
    ...
    submit: function (formName) {
      ...
      if (canSubmit) {
        ...
        $.ajax({
          type: 'GET',
          url: 'http://localhost:8081/Hello/login_request',
          crossDomain: true,
          xhrFields: {
            withCredentials: true
          },
      ...
      if (canSubmit) {
        $.ajax({
          type: 'GET',
          url: 'http://localhost:8081/Hello/submit_login',
          crossDomain: true,
