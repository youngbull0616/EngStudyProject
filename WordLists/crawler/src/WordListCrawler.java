...
public class WordListCrawler {
    private final static String word_books[] = {
            "/Users/youngbull/Desktop/BS-Project-master/WordLists/crawler/src/六级词汇.txt",
    };
    private final static String word_books_path[] = {"CET-6.xml"};
    ...
    private static void DOMCreate(File file) {
        try {
            ...
            for (WordEntry w: WordEntries) {
                ...
                poses.setTextContent(translation(w.word));
    ...
    private static String translation(String word) {
        String poses = null;
        try {
            // 构造查询 URL
            String url = "http://dict.youdao.com/w/eng/" + word;

            // 使用 Jsoup 进行网页爬取
            org.jsoup.nodes.Document doc = Jsoup.connect(url).get();

            // 解析翻译结果
            Elements transContainers = doc.select(".trans-container");
            if (transContainers.size() > 0) {
                org.jsoup.nodes.Element transContainer = transContainers.get(0);
                Elements translations = transContainer.select("li");
                if (translations.size() > 0) {
                    StringBuilder translationBuilder = new StringBuilder();
                    for (org.jsoup.nodes.Element translation : translations) {
                        translationBuilder.append(translation.text()).append("\n");
                    }
                    poses = translationBuilder.toString().trim();
                } else {
                    poses = "未找到翻译结果";
                }
            } else {
